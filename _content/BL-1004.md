...
Information is processed data that has meaning and value. It includes things like names, addresses, passwords, financial records, and messages. For example, if you write down someone’s phone number, that’s data, but if you save it as a contact with their name, it becomes information because it is useful.

## What is Information Security? {#What-is-Information-Security}
Information security (InfoSec) is protecting information from being stolen, changed, or destroyed. It ensures that only the right people can access the right information at the right time.

## What is Security? {#What-is-Security}
Security means protection from harm or danger. In cybersecurity, security means protecting computers, networks, and data from attacks, hackers, and malware.
...
#### Mandatory Access Control (MAC) vs. Discretionary Access Control (DAC) vs. Role-Based Access Control (RBAC){#Mandatory-Access-Control-MAC-vs.-Discretionary-Access-Control-DAC-vs.-Role-Based-Access-Control-RBAC}
...
- <b>RBAC:</b> Access is based on a user's role within an organization (used in companies and enterprise systems).

<b>Example:</b> In a MAC system, only the administrator can assign access levels. In a DAC system, a file owner can choose who can read or edit their files. In an RBAC system, an employee is assigned a role (e.g., "Manager"), and that role determines their access permissions.
...
## Threat vs. Risk vs. Attack {#Threat-vs-Risk-vs-Attack}
In cybersecurity, threats and risks are often confused, but they mean different things:
- A **threat** is something that can cause harm (e.g. hackers, malware, or a phishing attack).
- A **risk** is the chance of harm happening (how likely the threat is to cause damage).
- A **attack** is when a threat actively tries to exploit a system (e.g., a hacker launching a phishing campaign).


Simply put:
- **Threat = Danger** ((e.g., hackers, malware))
- **Risk = Chance of being affected** (e.g., weak passwords increase the risk of hacking)
- **Attack = Action taken by a threat to cause harm** (e.g., a hacker launching a phishing attack).
...
### Attacks {#Attacks}
An attack is when a threat turns into action—an actual attempt to exploit vulnerabilities.

**Examples of Attacks:**
- Phishing Attack → A hacker sends fake emails to steal login credentials.
- Malware Attack → A virus is installed on a system, stealing or corrupting data.
- DDoS Attack → Hackers flood a website with traffic, causing it to crash.
- Ransomware Attack → Files are encrypted, and attackers demand a ransom.

**Example:**
A hacker sending a phishing email with a fake login page is an attack.
